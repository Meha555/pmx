...
use crate::model::{Finding, FindingSeverity, METRIC_HANDLE_COUNT, MetricValue};
...
impl Analyzer for HandleGrowthAnalyzer {
    ...
    fn analyze(&self, ctx: &AnalyzeContext<'_>) -> Result<AnalyzeOutput> {
        let min_delta = super::option_u64(ctx.module, "min_delta").unwrap_or(10);
        ...
        for (target, values) in by_target {
            if let (Some(first), Some(last)) = (values.first(), values.last()) {
                if last > first && last - first >= min_delta {
