...
pub fn modules(args: ModulesArgs) -> Result<serde_json::Value> {
...
fn print_capabilities(capabilities: &[crate::model::CapabilityRequirement]) {
...
fn print_parameters(parameters: &[crate::modules::ParameterDescriptor]) {
...
fn print_metrics(metrics: &[crate::modules::MetricDescriptor]) {
    if metrics.is_empty() {
        return;
    }
    println!("  metrics:");
